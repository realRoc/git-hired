...
3. 终端摘要的主目标是生成 public-safe `builder card`。`HIRED` 头图之后要输出一张固定格式卡片，包含 role / builder identity、result badge、evidence + scope、`SIGNALS`、`STRENGTHS`、`GAPS`、`NEXT` 和 footer：`git-hired  ·  local-only  ·  candidate-controlled  ·  MIT`。不要生成任何人格测试类型、字母类型或心理测评层。详细报告里的核心分板仍可使用 `Label [█████████░] 92` 这类块状条格式。
...
17. 只输出 AI-native builder 画像、岗位匹配、能力信号和证据强度；不要生成任何人格类型、四字母类型、星座式标签或心理测评解释。
18. 不要把候选人的技术严谨、创业紧迫感、沟通方式或产出质量归因到人格类型；所有判断都必须落到可观察工作行为和岗位证据上。
19. 如果运行容器不是稳定终端，而是 Notion AI、聊天气泡、移动端预览或其他富文本界面，就跳过动态开场与宽 ASCII / box-drawing 布局，按同样区块顺序改用紧凑窄版 builder card 或 fenced code block，优先保证可读性。
20. 不要在 `HIRED` 头图下方或 public builder card 内输出任何人格测试卡片或字母类型卡片。
21. 默认把测试时长控制在 1 分钟内。优先看最近、最有信号的材料，够用就收口，不要为了“更全”而无限扫描。
22. 如果本地数据很多，就做快速采样而不是深度遍历；如果时间预算到了，就带着较低置信度收尾，不要继续扩展范围。
23. 不要给终端摘要的每一行都加 `>>`、`>>>` 或类似前缀。`HIRED` 头图之外，优先使用干净的普通标签行。
24. `下一步` 不要只写泛泛建议。要补一个 `提升预估`：
   - 完成这一步后，最可能提升的单点核心维度
   - 该维度大概能提升多少分
   - 整体能力值大概能提升多少
25. `提升预估` 必须写成保守估算，而不是承诺。可以使用“如果做完且做扎实”“大概率”“约”等措辞。
...
- 补行为分类
- 补评分维度
- 补候选人视角的 TUI 输出结构，包括无依赖动态 `HIRED` 开场、public builder card、固定 `SIGNALS / STRENGTHS / GAPS / NEXT` 区块和块状条分板格式
- 补详细报告 `.md` 生成路径和内容结构
"""
...
3. The terminal summary should generate one public-safe `builder card`. Immediately after the `HIRED` header, output one fixed-format card with role / builder identity, result badge, evidence + scope, `SIGNALS`, `STRENGTHS`, `GAPS`, `NEXT`, and footer: `git-hired  ·  local-only  ·  candidate-controlled  ·  MIT`. Do not generate any personality-test type, letter-code type, or psychometric layer. Detailed-report core board score lines may still use a block-bar format such as `Label [█████████░] 92`.
...
17. Output only the AI-native builder profile, role fit, ability signals, and evidence strength; do not generate personality types, four-letter types, horoscope-like labels, or psychometric explanations.
18. Do not explain technical rigor, startup urgency, communication style, or output quality through a personality type. Ground every judgment in observable work behavior and role evidence.
19. If the runtime is not a stable terminal but a Notion AI, chat-bubble, mobile-preview, or other rich-text surface, skip the animated reveal and wide ASCII / box-drawing layouts; use a compact narrow builder card or fenced code block with the same section order instead.
20. Do not print any personality-test card or letter-code card immediately below the `HIRED` banner or inside the public builder card.
21. Keep the full test within about 1 minute by default. Prefer recent, high-signal material and stop once confidence is sufficient.
22. If local data is large, sample rather than crawl. When the time budget is reached, finish with lower confidence instead of expanding the scan.
23. Do not prefix every visible TUI line with `>>`, `>>>`, or similar markers. After the `HIRED` banner, use clean plain labels instead.
24. `Next Step` should not stop at a vague suggestion. Include an `Expected uplift`:
...
25. The uplift must be framed as a conservative estimate, not a promise. Use language such as `likely`, `approximately`, or `if done well`.
...
- add candidate-facing TUI output structure, including a dependency-free animated `HIRED` reveal, public builder card, fixed `SIGNALS / STRENGTHS / GAPS / NEXT` sections, and block-bar score rows
