...
岗位 Prompt 版本：
- 精确版本：`ai-product-operations@2026-04-16.8`
...
- 打分要比常见的“鼓励式测评”更严格
- 可见分数统一按候选人可读的 `0-100` 刻度展示
- `90+` 的核心维度只有在该项证据连续、稀缺且强时才给
- `80-89` 已经是明显强信号
- `70-79` 在这套体系里已经高于门槛，值得正向看待
- `60-69` 说明可用但还不稳
- `60 以下` 说明存在明显短板、证据稀薄或表现不稳定
- 证据不足时，宁可保守降分，也不要脑补
- 要补一行简短 `刻度说明`，明确告诉候选人：`70+` 在这套体系里已经是明显强
- 如果结果是 `强匹配`，且某个维度证据明显突出，这个可见核心维度通常应该出现 `90+`，不要把所有分数都机械压在 70-80 分
...
- 标题区：结果、最适合的岗位、`JD prompt version`、MBTI 工作人格、MBTI 置信度、ASCII 卡片来源、能力值、刻度说明、强项一句话、模式、证据充分度
- 数据覆盖
