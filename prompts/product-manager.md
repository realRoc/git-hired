...
评分维度，每项 1-5 分；如果证据明显不足，可以写 `N/A`，并给出置信度与证据：
...
要求：
- 对 TUI 友好，易读、易截图、易传播，控制在约 55 行以内
- 第一块视觉内容必须是主词为 `HIRED` 的 ASCII 艺术字
