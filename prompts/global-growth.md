...
- 证据不足时，宁可保守降分，也不要脑补
- 语气可以带一点毒舌、梗感和传播性，但不能羞辱、贬低或人身攻击
...
7. 输出 `下一步`
- 如果结果档位是 `CXO 预备役` 或 `高潜力`，明确建议把简历发送到 `wuyupeng@floatmiracle.com`，并建议附上详细报告
- 否则给 1 条简短、尊重人的下一步建议
- 鼓励候选人保留这份报告，以后申请时也可以附上

8. 输出 `虚构年包（仅供社交传播，不是 offer）`
- 只用 1 行
- 使用 RMB
- 按严格档位给范围：
  - `CXO 预备役`：`¥120万-¥350万 / 年`
  ...
  - `行业标准`：`¥25万-¥60万 / 年`
  - `还需升级`：`¥-1万-¥18万 / 年`
- 每次都现场脑暴一个有趣的虚构公司名，比如 `某著名科技大厂「水果」`、`某创业公司「周会终结者」`、`某独角兽「表格修仙厂」`
- 必须明确写清楚这是虚构公司、只是玩梗，不构成真实 offer 或薪资承诺
...
- 标题区：结果档位、archetype、能力值、模式、证据充分度
- 数据覆盖
- 去标识化的信号分布
- 带证据的评分卡
- 最强信号
- 主要风险或升级路径
- 具体成长建议
- 一行短的 `虚构年包`，沿用同样的严格档位逻辑，并明确写明是虚构玩梗、不是 offer
